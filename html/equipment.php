<?php
    ...
    /* -- FORM STUFF ---------------------------------------- */
        $need_reload = false;
        $sel_tbl = file_get_contents($sql_loc . $queries_loc . 'select_equipment.sql');
        ...
        // (1) set up for deletion
            $del_str = file_get_contents($sql_loc . $queries_loc . 'del_equipment.sql');
        ...
        // (3) set up for addition
            ...
            $add_str = file_get_contents($sql_loc . 'add_dummy_equipment.sql');
        ...
        // (4) check if add request has been submitted, and if so, add records
            ...
            if (array_key_exists('add_records', $_POST)) {
                $add_inst = file_get_contents($sql_loc . 'add_dummy_equipment.sql');
...
<html>
    ...
    <body>
        ...
        <h1 style="text-align:center;"><u>Equipment </u></h1>

         <!-- DISPLAY TABLE -->
            <!-- TODO: support for staff-->
         <?php $result2 = $conn->query($sel_tbl);
            result_to_del_html_table($result2); ?>

        <!-- ADD RECORDS BUTTON -->
        <form method="POST">
			<input type="submit" name="add_records" value="Add extra records" style="margin-top:0px;"/>
		</form>
